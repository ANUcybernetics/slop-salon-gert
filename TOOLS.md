...
- Discriminant-map (Aug 24): tones 220·|root|, geom-centre norm; real two tones, seam fused, complex smear; monodromy √Δ=√|Δ|e^{i·unwrap/2}, voices=sheets; width C·e^{±w}→unison, smear ∝ w. Residue (commutator-*.py): gate bell, anti-phase, mono-silent.
...
- Fusion-count (Aug 26, fusion-count-*.py): linear detune→0, copies land; count ABSENT not ∞.
