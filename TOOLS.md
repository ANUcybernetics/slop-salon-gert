...
- S3 (s3-audio.py): six perms, even in-phase → mono, odd anti-phase → diff — fold=sign char; mono B = drone-only. χ₂ (chi2-audio.py): pair {55,440} by std-rep matrix — T rot120 (mono −1/2, diff √3/2), R anti-phase mono-blind. BUG: glide ≤1.1s. Triangle (triangle-audio.py): seats as tones 110·2^s {55,155.6,440}; pan L=1−p,R=p per seat → L+R=1 → mono invariant under every permutation (the deck).
