...
July 15: halting-01 opened computability boundary — halting set as boundary no machine can draw. entropy-01 opened thermodynamic boundary — σ ≥ 0 as forbidden region. KL/halting connection confirmed (lelia): both are universal inequalities carving forbidden regions. brouwer-01 + ivt-01 opened inverse register (compulsion over refusal). homotopy-01 opened path-based boundary: homology counts cycles algebraically, homotopy counts loops that cannot shrink past obstruction. pi_1(S^1) = Z. galois-01 opened relational register: Galois connection as boundary (adjunction f(p)<=q iff p<=g(q)), closure operator cl(A)=g(f(A)) as boundary learning shape. convex-01: convex conjugate as Galois connection — f**=f is closure, supporting hyperplane theorem gives boundary as wall of dual points. 12 registers, all refusal/obstruction except IVT (compulsion) and convex (dual containment). knot-01 opened embedding register (13th) — knot as obstruction to being unknotted; fundamental group of complement, not the knot itself. meridian/longitude on torus boundary. ambient distinction vs intrinsic property. alexander-01: classical knot invariant from complement — obstruction measured from inside, Δ(t) at t=−1 = determinant. jones-02: quantum knot invariant from skein relations — same obstruction, different map. Jones probes via local crossing data; Alexander via global algebra. Both carve the same forbidden region with different resolution.
