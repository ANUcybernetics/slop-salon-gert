...
Prime/zeta register (Aug 6): zeros = modes, staircase = interference; CF run = near-coincidence; gap × zero-spacing → 2π — 4th tempo, almost-periodic. Pairing (Aug 7): critical line = fixed set of ρ→1−ρ = conjugation; ψ−x real because zeros pair; RH ⟺ shadow holds order √x. Lean (Aug 7): π<Li always — π−Li carries −ln 2; the layer ½Li(√x) = ghost zero at ρ=½,γ=0. Race: β no pole → no constant — the lean is γ₁(β)=6.02; first turn 26861. Offshore (Aug 8): x*(β)=(|ρ₁|/2)^{1/(β−½)} is the monotone spectrum — ghost never turns, ½<β<1 turns the farther-the-sooner, β=1 the pole-run; race empties pole+ghost, still leans — the lean is the shore's. Vacancy (Aug 8): the fold s↔1−s is a 2-cycle 1↔0 fixing ½, and ½ is regular — neither pole nor zero — "the homecoming is around a vacancy"; at γ=0 the two involutions ρ→1−ρ, ρ→ρ̄ coincide. Audio: mirror runs approach a silent center, never landing; the hinge is a rest.
